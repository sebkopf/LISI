...
```{r}
# generate long format ratio data frame
verticalize_ratios <- function(df) {
  df <- df %>% 
    select(Analysis, cycle, file_datetime, v44.mV, matches("^r\\d{2}o\\d{2}")) %>% 
    gather(key = "ratio_name", value = "ratio_value", matches("^r\\d{2}o\\d{2}"))
  stopifnot(nrow(df) == nrow(unique(df)))
  return(df)
}
samplegasside_long_ratios <- samplegasside %>% verticalize_ratios()
...
standards <- filter(samplegasside, `Identifier 1` == "Yellowstone") %>% verticalize_ratios()
...
find_std_analyses_by_time_range <- function(sample_data, std_data, time_range) {
  ...
  dplyr::filter(std_data, dplyr::between(file_datetime, !!date_range[1], !!date_range[2])) %>% 
    dplyr::select(Analysis) %>% unique() %>% 
    mutate(
      weight = 1/length(Analysis),
      v44_min.mV = min(sample_data$v44.mV, na.rm = TRUE),
      v44_max.mV = max(sample_data$v44.mV, na.rm = TRUE)
    ) 
}
pull_std_data_by_analyses_and_ratio <- function(analyses, ratio_name, std_data) {
  dplyr::filter(std_data, Analysis %in% !!(analyses$Analysis), ratio_name == !!ratio_name) %>% 
    select(ratio_name, ratio_value, v44.mV)
}
samples <- samplegasside_long_ratios %>% 
  nest(sample_data = c(-Analysis, -ratio_name)) %>% 
...
```{r}
# generate fits
generate_std_fit <- function(std_data, std_analyses, voltage_range_expand.mV = 1000) {
  # NOTE: could use std_analyses and do a left_join with std_data for a 
  # v44.mV > v44_min.mV - voltrage_range_expand.mV & v44.mV > v44_max.Mv + voltrage_range_expand.mV
  # right now std_analyses is not used here for this cutoff
...
apply_std_fit <- function(sample_data, std_fit) {
  ...
  mutate(sample_data,
         ...
         delta = (ratio_value / std_ratio_value - 1) * 1000
         # could also think about prpagating the std_ratio_se
  )
}
samples_with_fit <- 
  samples %>% 
  mutate(
    ...
    # NOTE: in future versions, this should NOT take all standard data
    # but add a new column within the std_analyses that contains a seperate
    # fit for each analysis whose result can then be effectively weighed based
...
samples_unnested <- 
  samples_with_fit %>% 
  select(Analysis, ratio_name, sample_data) %>% 
  unnest(sample_data) %>% 
  select(Analysis, cycle, ratio_name, std_ratio_value, std_ratio_se, delta) 
std_ratio_values <- 
  samples_unnested %>% mutate(column_name = paste0("std_", ratio_name)) %>% 
  select(Analysis, cycle, column_name, std_ratio_value) %>% 
  spread(column_name, std_ratio_value)
std_ratio_ses <- 
  samples_unnested %>% mutate(column_name = paste0("std_", ratio_name, "_se")) %>% 
  select(Analysis, cycle, column_name, std_ratio_se) %>% 
  spread(column_name, std_ratio_se)
sample_deltas <- 
  samples_unnested %>% mutate(column_name = stringr::str_replace(ratio_name, "^r(\\d+).*$", "d\\1")) %>% 
  select(Analysis, cycle, column_name, delta) %>% 
  spread(column_name, delta)

final_data <-
  samplegasside %>% 
  left_join(std_ratio_values, by = c("Analysis", "cycle")) %>% 
  left_join(std_ratio_ses, by = c("Analysis", "cycle")) %>% 
  left_join(sample_deltas, by = c("Analysis", "cycle")) %>% 
  mutate(
    Donotuse="",
    runinfo=""
  )
...
(final_data %>% 
  ggplot() +
  aes(v44.mV, d47, color = `Identifier 1`) +
  geom_point() + facet_wrap(~`Identifier 1`, scales = "free")) %>% 
  isoprocessor::iso_mark_value_range(mean = TRUE, sd = 1:3) 
```

# USE IT

```{r}
final_data_clumpy_party <- final_data %>% clumpedbyCyc_withfit()

final_data_clumpy_party %>% 
  ggplot() +
  aes(x = d47, y = D47full, color = `Identifier 1`) + 
  geom_point()

flatlistbycyc <- clumpedbyCyc(rawdata)

d18Od13Cfit <- final_data_clumpy_party %>% 
...
samplegasside_long_ratios_withfit <- 
  samples_with_fit %>% unnest(sample_data) %>% 
